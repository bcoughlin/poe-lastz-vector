...
def log_interaction_to_console(interaction_data: dict[str, Any]):
    ...
    if interaction_data["interaction"]["image_data"]:
        for i, img in enumerate(interaction_data["interaction"]["image_data"]):
            print(
                f"   Image {i + 1}: {img.get('content_type', 'unknown')} - {img.get('name', 'unnamed')}"
            )
    ...
    print(
        f"📝 Response Length: {interaction_data['interaction']['bot_response_length']} chars"
    )
...
def download_and_store_image(
    ...
    try:
        ...
        print(
            f"✅ Downloaded and stored image: {filepath} ({len(response.content)} bytes)"
        )
        return filepath
